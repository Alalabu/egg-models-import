 ...
 */
const loadRemoteModels = async (sequelize, modelHost, modelIn, modelAttrs) => {
  // 1. 从外部动态获取 models 列表
  const sequelize_models = await HttpClient.$http(`${modelHost}${modelIn}`, 'POST');
  if (!Array.isArray(sequelize_models)) {
    return console.error('sequelize mdoels load error!');
  }
  // Sequelize 数据类型(重命名)
  ...
  for (const model_name of sequelize_models) {
    ...
    const model_res = await HttpClient
      .$http(`${modelHost}${modelAttrs}`, 'POST', { tableName: model_name });
    // 缓存的临时模型
    const model_tmp = {};
...
module.exports = {
  get model_promise() {
    if (!this[MOCKMODEL]) {
      ...
      const { modelHost, modelIn, modelAttrs } = modelExport;
      ...
      this[MOCKMODEL] = loadRemoteModels(_sequelize, modelHost, modelIn, modelAttrs);
    }
